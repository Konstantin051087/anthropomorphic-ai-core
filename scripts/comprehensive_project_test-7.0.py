#!/usr/bin/env python3
"""
КОМПЛЕКСНЫЙ ТЕСТ ВСЕГО ПРОЕКТА ANTHROPOMORPHIC AI - ВЕРСИЯ 7.0 С ИСПРАВЛЕННЫМ ОБНАРУЖЕНИЕМ ЭНДПОИНТОВ
"""
...
class ComprehensiveProjectTest:
    ...
    def _analyze_routes_with_ast(self, content: str, expected_endpoints: List[str]) -> List[Dict]:
        """Улучшенный AST-анализ для обнаружения эндпоинтов"""
        endpoints_found = []
        ...
        try:
            ...
            for node in ast.walk(tree):
                # Ищем декораторы с маршрутами
                if isinstance(node, ast.FunctionDef):
                    for decorator in node.decorator_list:
                        if isinstance(decorator, ast.Call):
                            # Получаем функцию декоратора
                            func = decorator.func
                            
                            # Проверяем разные форматы декораторов
                            if (isinstance(func, ast.Attribute) and 
                                hasattr(func, 'attr') and
                                func.attr in ['get', 'post', 'put', 'delete']):
                                ...
                                for arg in decorator.args:
                                    ...
                                    if isinstance(arg, ast.Str):  # Python < 3.8
                                        endpoint_path = arg.s
                                          ...
                                          isinstance(arg.value, str)):  # Python >= 3.8
                                        endpoint_path = arg.value
                                    
                                    if endpoint_path:
                                        # Сравниваем с ожидаемыми эндпоинтами
                                        for expected in expected_endpoints[:]:
                                            if (endpoint_path == expected or 
                                                endpoint_path.startswith(expected) or
                                                expected in endpoint_path):
                                                self.print_result(
                                                    f"API эндпоинт (AST): {expected}",
                                                    True,
                                                    f"Найден как: {endpoint_path} ({func.attr.upper()})"
                                                )
                                                ...
                                                if expected in expected_endpoints:
                                                    expected_endpoints.remove(expected)
                                                break
                            
                            # Также проверяем router.get/post и т.д.
                            elif (isinstance(func, ast.Name) and 
                                  func.id == 'router' and decorator.args):
                                # Это может быть @router("/path")
                                for arg in decorator.args:
                                    if isinstance(arg, ast.Str):
                                        endpoint_path = arg.s
                                        for expected in expected_endpoints[:]:
                                            if expected in endpoint_path:
                                                self.print_result(
                                                    f"API эндпоинт (AST): {expected}",
                                                    True,
                                                    f"Найден в router: {endpoint_path}"
                                                )
                                                endpoints_found.append({
                                                    "endpoint": expected,
                                                    "implemented": True,
            ...
            logger.error(f"Ошибка AST анализа: {e}")
            self.print_result("AST анализ routes.py", False, f"Ошибка: {e}")
            # В случае ошибки используем резервный метод
            return self._analyze_routes_with_regex_fallback(content, expected_endpoints)
        
        return endpoints_found

    def _analyze_routes_with_regex_fallback(self, content: str, expected_endpoints: List[str]) -> List[Dict]:
        """Улучшенный резервный метод поиска эндпоинтов"""
        endpoints_found = []
        ...
        for endpoint in expected_endpoints:
            ...
            patterns = [
                f'@router\\.(get|post|put|delete)\\(["\\\']{re.escape(endpoint)}["\\\']',
                f'@.*\\.(get|post|put|delete)\\(.*["\\\']{re.escape(endpoint)}["\\\']',
                f'["\\\']{re.escape(endpoint)}["\\\'].*\\n.*@.*\\.(get|post|put|delete)',
                f'@router\\(["\\\']{re.escape(endpoint)}["\\\']\\)'
            ]
...
def main():
    """Основная функция"""
    print("🚀 ЗАПУСК ВЕРСИИ 7.0 КОМПЛЕКСНОГО ТЕСТА С ИСПРАВЛЕННЫМ ОБНАРУЖЕНИЕМ ЭНДПОИНТОВ")
