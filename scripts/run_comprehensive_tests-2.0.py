...
Объединенный скрипт для проверки структуры проекта, API и всех компонентов системы
"""
...
class ComprehensiveProjectTest:
    ...
    def _test_subtask_1_1_3(self):
        ...
        if gitignore_file.exists():
            try:
                ...
                missing_patterns = [pattern for pattern in critical_patterns if pattern not in content]
    ...
    def _test_subtask_1_2_1(self):
        ...
        # Проверка эндпоинтов в routes.py
        ...
        if routes_file.exists():
            try:
                ...
                for endpoint in expected_endpoints:
                    found = f'"/{endpoint}"' in content or f"/{endpoint}" in content
                    check_result = self.print_result(
    ...
    def _test_subtask_1_3_3(self):
        ...
        if runtime_file.exists():
            try:
                ...
                is_correct = content == "python-3.13.4"
                check_result = self.print_result(
                    "Файл runtime.txt",
                    is_correct,
                    f"Содержимое: {content}" if is_correct else f"Ожидается python-3.13.4, найдено: {content}"
                )
                subtask_results["checks"].append({
                    "check": "runtime_txt",
                    "exists": True,
    ...
    def generate_recommendations(self):
        ...
        if task_1_2.get("status") != "COMPLETED":
            recommendations.append("🏗️ Задача 1.2: Завершите проектирование архитектуры")
        
        # Задача 1.3
        task_1_3 = tasks.get("1.3", {})
        if task_1_3.get("status") != "COMPLETED":
            recommendations.append("⚙️ Задача 1.3: Завершите настройку инфраструктуры для деплоя")
        ...
        if not self._test_api_availability():
            recommendations.append("🌐 Запустите API сервер: python api/app.py")
...
class APITestRunner:
    """Класс для тестирования API endpoints и системы"""
    ...
    def run_server(self):
        ...
        try:
            server_script = self.base_dir / "api" / "app.py"
            if server_script.exists():
                ...
                print("✗ Файл сервера не найден: api/app.py")
                return False
        except Exception as e:
    ...
    def run_unit_tests(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"✗ Unit-тесты не пройдены: {result.returncode}")
                print(result.stdout)
                print(result.stderr)
                return False
    ...
    def check_dependencies(self):
        ...
        dependencies = ["fastapi", "uvicorn", "sqlalchemy", "pydantic", "requests", "pytest"]
    ...
    def run_comprehensive_tests(self):
        ...
        # Финальный отчет
        print("\n" + "="*60)
        if api_success and unit_success:
...
def main():
    """Основная функция объединенного скрипта"""
    parser = argparse.ArgumentParser(description='Комплексное тестирование проекта Anthropomorphic AI')
    parser.add_argument('--mode', choices=['project', 'api', 'all'], default='all',
                       help='Режим тестирования: project (структура), api (endpoints), all (полный)')
    parser.add_argument('--no-input', action='store_true', help='Не запрашивать подтверждение')
    
    args = parser.parse_args()
    
    print("🚀 ОБЪЕДИНЕННЫЙ ТЕСТ ПРОЕКТА ANTHROPOMORPHIC AI")
    print("Этот скрипт проверит структуру проекта, API endpoints и все компоненты системы")
    ...
    # Запуск тестов структуры проекта
    if args.mode in ['project', 'all']:
        print("\n" + "="*80)
        print("🏗️  ТЕСТИРОВАНИЕ СТРУКТУРЫ ПРОЕКТА")
        print("="*80)
        
        project_tester = ComprehensiveProjectTest()
        project_success = project_tester.run_comprehensive_test()
        overall_success = overall_success and project_success
    
    # Запуск тестов API и системы
    if args.mode in ['api', 'all']:
        print("\n" + "="*80)
        print("🌐 ТЕСТИРОВАНИЕ API И СИСТЕМЫ")
        print("="*80)
        
        api_tester = APITestRunner()
        api_success = api_tester.run_comprehensive_tests()
    ...
    if overall_success:
        ...
        print("Проект готов к использованию и деплою!")
        sys.exit(0)
    else:
        print("🔧 ТЕСТИРОВАНИЕ ВЫЯВИЛО ПРОБЛЕМЫ")
        print("Пожалуйста, исправьте выявленные проблемы и запустите тесты снова")
        sys.exit(1)
